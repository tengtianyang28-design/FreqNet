...
from custom_modules.dual_path_refiner import DualPathFPNRefinement
from custom_modules.prior_knowledge import PriorKnowledgeFilter
from detectron2.layers import ShapeSpec
from detectron2.modeling.box_regression import Box2BoxTransform
from torchvision.ops import nms
from custom_modules.frequency_refinement import FrequencyGuidedFPNRefinement
...
class FixMatchGeneralizedRCNN(GeneralizedRCNN):
    ...
    def __init__(
        ...
        *,
        ...
        frequency_refinement_gate: bool = True,
        frequency_refinement_levels: Optional[List[str]] = None,
        # Dual-path refinement (Innovation Point 2)
        use_dual_path_refinement: bool = False,
        dual_path_kernel_size: int = 3,
        dual_path_num_heads: int = 4,
        dual_path_dropout: float = 0.1,
        dual_path_reduction_ratio: int = 4,
        dual_path_refinement_levels: Optional[List[str]] = None,
        prior_filter_cfg=None,
        ...
        self.use_dual_path_refinement = use_dual_path_refinement

        self.prior_filter = None

        if self.use_frequency_refinement:
            backbone_output_shape = backbone.output_shape()
            if backbone_output_shape:
                sample_key = next(iter(backbone_output_shape))
                sample_shape = backbone_output_shape[sample_key]
                if hasattr(sample_shape, "channels") and sample_shape.channels is not None:
                    fpn_channels = sample_shape.channels
                elif isinstance(sample_shape, (list, tuple)) and len(sample_shape) > 0:
                    fpn_channels = sample_shape[0]
                else:
                    fpn_channels = 256
                fpn_levels = list(backbone_output_shape.keys())
            else:
                fpn_channels = 256
                fpn_levels = ['p2', 'p3', 'p4', 'p5', 'p6']

            enabled_levels = frequency_refinement_levels if frequency_refinement_levels else None
            if enabled_levels is not None:
                enabled_levels = [lvl for lvl in enabled_levels if lvl in fpn_levels]
                if not enabled_levels:
                    enabled_levels = None

            self.frequency_refinement = FrequencyGuidedFPNRefinement(
                in_channels=fpn_channels,
                fpn_levels=fpn_levels,
                use_gate=frequency_refinement_gate,
                enabled_levels=enabled_levels
            )
        else:
            self.frequency_refinement = None

        if self.use_dual_path_refinement:
            backbone_channels = self.backbone.output_shape()
            fpn_channels = list(backbone_channels.values())[0].channels if backbone_channels else 256
            fpn_levels = list(backbone_channels.keys()) if backbone_channels else ['p2','p3','p4','p5','p6']
            self.dual_path_refinement = DualPathFPNRefinement(
                in_channels=fpn_channels,
                fpn_levels=fpn_levels,
                kernel_size=dual_path_kernel_size,
                num_heads=dual_path_num_heads,
                dropout=dual_path_dropout,
                reduction_ratio=dual_path_reduction_ratio,
                enabled_levels=dual_path_refinement_levels
            )
        else:
            self.dual_path_refinement = None

        if prior_filter_cfg is not None and getattr(prior_filter_cfg, "ENABLED", False):
            class_names = class_names or []
            self.prior_filter = PriorKnowledgeFilter(
                enabled=True,
                prior_path=prior_filter_cfg.PATH,
                class_names=class_names,
                high_vote=prior_filter_cfg.HIGH_VOTE,
                medium_vote=prior_filter_cfg.MEDIUM_VOTE,
                medium_score=prior_filter_cfg.MEDIUM_SCORE,
                min_score=prior_filter_cfg.MIN_SCORE,
            )
        
    @staticmethod
    def _parse_frequency_levels(levels):
        """Parse frequency refinement levels from config, handling string/list inputs."""
        if levels is None:
            return None
        if isinstance(levels, str):
            try:
                ...
                levels = [lvl.strip().strip("'\"") for lvl in levels.split(',') if lvl.strip()]
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            "use_frequency_refinement": getattr(cfg.MODEL, "USE_FREQUENCY_REFINEMENT", False),
            "frequency_refinement_gate": getattr(cfg.MODEL, "FREQUENCY_REFINEMENT_GATE", True),
            "frequency_refinement_levels": cls._parse_frequency_levels(
                getattr(cfg.MODEL, "FREQUENCY_REFINEMENT_LEVELS", None)
            ),
            # Dual-path refinement flags
            "use_dual_path_refinement": getattr(cfg.MODEL, "USE_DUAL_PATH_REFINEMENT", False),
            "dual_path_kernel_size": getattr(cfg.MODEL, "DUAL_PATH_KERNEL_SIZE", 3),
            "dual_path_num_heads": getattr(cfg.MODEL, "DUAL_PATH_NUM_HEADS", 4),
            "dual_path_dropout": getattr(cfg.MODEL, "DUAL_PATH_DROPOUT", 0.1),
            "dual_path_reduction_ratio": getattr(cfg.MODEL, "DUAL_PATH_REDUCTION_RATIO", 4),
            "dual_path_refinement_levels": cls._parse_dual_path_levels(
                getattr(cfg.MODEL, "DUAL_PATH_REFINEMENT_LEVELS", None)
            ),
            "prior_filter_cfg": cfg.PRIOR_FILTER.clone() if hasattr(cfg, "PRIOR_FILTER") else None,
            "class_names": class_names,
        }

    @staticmethod
    def _parse_dual_path_levels(levels):
        if levels is None:
            return None
        if isinstance(levels, str):
            try:
    ...
    def forward(self, batched_inputs):
        ...
        if self.use_frequency_refinement and self.frequency_refinement is not None:
            wa_features = self.frequency_refinement(wa_features)
            sa_features = self.frequency_refinement(sa_features)
        if self.use_dual_path_refinement and self.dual_path_refinement is not None:
            wa_features = self.dual_path_refinement(wa_features)
            sa_features = self.dual_path_refinement(sa_features)
        ...
        for i in range(len(batched_inputs)):
            if i in present_ids:
                ...
                pdb.set_trace()
        if self.prior_filter is not None and len(wa_pred_instances):
            wa_pred_instances = self.prior_filter.filter_batch(batched_inputs, wa_pred_instances)
        sa_appended_gt = []
        ...
        if self.vis_period > 0:
            ...
            if storage.iter % self.vis_period == 0:
                self.visualize_training(batched_inputs, wa_proposals)
        losses = {}
